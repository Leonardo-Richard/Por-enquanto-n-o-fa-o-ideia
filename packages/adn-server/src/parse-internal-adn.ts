...
import { adnJsonError } from "./adn-json-error";
import { requireAdnWorkerSecret, verifyAdnWorkerHmacHeaders } from "./adn-hmac";
import {
...
import { adnJsonFromZodError } from "./adn-zod-response";
...
> {
  ...
  if (!secret) {
    return { ok: false, response: adnJsonError(503, "Serviço ADN interno não configurado.") };
  }
  ...
  if (!v.ok) {
    return { ok: false, response: adnJsonError(401, "Não autorizado.") };
  }
