...
export type MonitoredCompanyRowProps = {
  ...
  /** Último job da org (overview) — rótulo sem GET /adn/sync por linha. */
  lastJobOverview?: AdnExecutionsOverviewLastJob;
};

export function MonitoredCompanyRow({
  company,
  effectiveOrganizationId,
  lastJobOverview,
}: MonitoredCompanyRowProps) {
  ...
  if (!orgMatches) {
    ...
    return <MonitoredCompanyRowBody company={company} lastJobOverview={lastJobOverview} />;
  }

  return (
    <MonitoredCompanyRowWithAdn company={company} lastJobOverview={lastJobOverview} />
  );
}

function MonitoredCompanyRowWithAdn({
  company,
  lastJobOverview,
}: {
  company: MonitoredCompanyRow;
  ...
  return (
    <MonitoredCompanyRowBody company={company} adn={adn} lastJobOverview={lastJobOverview} />
  );
}
...
function MonitoredCompanyAdnRowActions({ adn }: { adn: AdnState }) {
  const { access, busy, actionMsg, actionTone, refresh, requestSync } = adn;

  return (
    <div className="flex w-full max-w-md flex-col gap-2 sm:items-end">
      {access === "loading" ? (
        <p className="text-xs text-black/50 dark:text-white/45">A verificar permissões ADN…</p>
      ) : null}
      ...
      {access === "error" ? (
        <div className="flex flex-col items-stretch gap-2 sm:items-end">
          <p className="text-xs text-red-800 dark:text-red-300" role="alert">
            Não foi possível verificar permissões ADN.
          </p>
          <button
            type="button"
            disabled={busy}
