...
export async function POST(request: Request) {
  try {
    ...
    const [org] = await db
      ...
      .limit(1);
    if (!org) return jsonError(403, "Operação não permitida.");
    if (!org.adnSyncEnabled) return jsonError(403, "Sincronização ADN desactivada para esta organização.");
    ...
    const [co] = await db
      ...
      .limit(1);
    if (!co) return jsonError(403, "Operação não permitida.");
    ...
    if (!draft) return jsonError(500, "Não foi possível criar rascunho de artefacto.");
