...
declare global {
  ...
  var __portalApiDb: ReturnType<typeof createDb> | undefined;
}
...
function requireDatabaseUrl(): string {
  ...
  if (/^https?:\/\//i.test(trimmed)) {
    throw new Error(
      "DATABASE_URL não pode ser https://… (URL do projeto). No Supabase use a URI Postgres: postgresql://…",
    );
  }
  return trimmed;
}
...
export function getDbInstance() {
  if (!globalThis.__portalApiDb) {
    globalThis.__portalApiDb = createDb(requireDatabaseUrl());
  }
  return globalThis.__portalApiDb;
}
